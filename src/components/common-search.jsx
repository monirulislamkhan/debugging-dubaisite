...
export default function CommonSearch({ onClick, parentClass = 'max-w-3xl mx-6', inputClasses = '' }) {
			...
			<form className={`${parentClass} relative w-full`}>
				...
				renderMenuItemChildren={options =>
					<div className="w-full flex gap-x-3">
						<span className="grow text">{options.label}</span> <span className="opacity-50">{options.type}</span>
					</div>
				}
				/>
			</form>
